...
class add extends top
{
	...
	public function post_state()
	{
		...
		spClass('db_member')->incrField(array('uid'=>$_SESSION['uid']),'num'); //如果不是编辑的话就加
		prient_jump(spUrl('main'));
	}
		...
		if(is_array($image))
		{
			...
			{			
				...
				if($i > $this->dtk['addimg_count']) //如果超过后台设定张数则把超过的删除
				{
					...
					$rs = spClass('db_attach')->find(array('id'=>$k),'','id,path,width,height');
					if(is_array($rs)) //如果记录验证成功
					{
						...
						if(file_exists($thum))
						{
							$data[] = array('url'=>$thum,'desc' =>$desc, 'width'=>$rs['width'], 'height'=>$rs['height']);
						}else{
							
							$data[] = array('url'=>$rs['path'],'desc' =>$desc, 'width'=>$rs['width'], 'height'=>$rs['height']);
						}
