...
class openconnect extends top
{
	...
	public function qq()
	{
		if($this->dtk['openlogin_qq_open'] == 0){exit('系统管理员没有开启QQ登陆功能');}
		$this->app = 'qq';
		spClass('qqConnect')->init($this->dtk['openlogin_qq_appid'],$this->dtk['openlogin_qq_appkey'],$this->dtk['openlogin_qq_callback']);
	...
	{
		if($this->dtk['openlogin_weib_open'] == 0){exit('系统管理员没有开启weibo登陆功能');}
		$this->app = 'weibo';
		...
		if($this->spArgs('callback'))
		{
			if($sina->LoginCallback()){	header("Location:index.php?c=openconnect&a=weibo&login=yes");}	exit;
		}
		
	
		if($this->spArgs('login'))
		{
			if(!$_SESSION['weibo']['result']){exit('<a href="javascript:window.history.go(-1)">登陆状态失效,请尝试后退操作</a>');}
			$type = 'WEIB';  //获取类型为新浪微博
			$this->user = $_SESSION['weibo']['result'];   //[oauth_token]  [oauth_token_secret] [openid]
			$sinauser = $sina->getUserInfo($_SESSION['weibo']['result']);
			$pic = str_replace('/50/','/180/',$sinauser['profile_image_url']);
			$this->type = $this->spArgs('type','reg');
			...
			if($this->spArgs('linkSubmit'))
			{
				if($this->spArgs('type') != 'login')
				{
					...
					if( false == $userobj->spVerifier($this->spArgs()) ){  
						$uid = $userobj->userReg($this->spArgs());
						$this->activeLogin($this->user['openid'],$this->user['oauth_token'],$this->user['oauth_token_secret'],$uid,$type);
						if($this->spArgs('face')) $this->getUserFace($pic,$uid);
						$this->jslocation(spUrl('main','index'));
					}else{
						$this->errmsg_arr = $userobj->spVerifier($this->spArgs());	
					}
					...
					$userobj->verifier = $userobj->verifier_openConnect_Login; 
					if( false == $userobj->spVerifier($this->spArgs()) ){  
						$this->activeLogin($this->user['openid'],$this->user['oauth_token'],$this->user['oauth_token_secret'],$_SESSION['uid'],$type);
						if($this->spArgs('face')) $this->getUserFace($pic,$_SESSION['uid']);
						$this->jslocation(spUrl('main','index'));
					}else{
						$this->errmsg_arr = $userobj->spVerifier($this->spArgs());	
					}
