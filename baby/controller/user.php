...
{ 
	...
	{
		if($this->dtk['keep_niname'] != '')
		{
			$arr = explode(',',$this->dtk['keep_niname']);
			if(in_array($this->spArgs('niname'),$arr)) {js_err('该昵称被保留或限制');}
		}
		
		if($this->dtk['keep_domain'] != '')
		{
			$arr = explode(',',$this->dtk['keep_domain']);
			if(in_array($this->spArgs('domain'),$domainKeep))  {js_err('该个性域名被保留或限制');} 
		}
		...
		$row = array('username'=>htmlspecialchars($this->spArgs('niname')),
					...
					//yoho
					 'sex'=>$this->spArgs('sex'),
					 'career'=>$this->spArgs('career'),
					 'year'=>$this->spArgs('year'),
					 'month'=>$this->spArgs('month'),
					 'day'=>$this->spArgs('day'),
					 'province'=>$this->spArgs('province'),
					 'city'=>$this->spArgs('city'),
					 'area'=>$this->spArgs('area')
					 );
