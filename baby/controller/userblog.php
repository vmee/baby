...
class userblog extends top
{
	...
	public function show()
	{
		...
		$tmpblog = spClass('db_blog')->spLinker()->find(array('uid'=>$this->user_data['uid'],'bid'=>$this->spArgs('bid')));
		$body = split_attribute($tmpblog["body"]);
		$imgs = $body["attr"]["img"];
		$imgso = $imgs[0];
		$results = spClass('db_replay')->spLinker()->spPager(1,5)->findAll(array('bid'=>$this->spArgs('bid')),'time desc','');
		foreach($comments as $index => $comment)
		{
			$body = split_attribute($results[$index]["blog"]["body"]);
			//$img = $body["attr"]["img"][0]["url"]?$body["attr"]["img"][0]["url"]:"";
			$tmpResult[] = array(
									//用户信息
									"uid" => $comments[$index]["uid"],
									"username" => $comments[$index]["user"]["username"],
									"avatar" => get_avatar($comments[$index]["uid"]),
									//内容
									"id" => $comments[$index]["id"],//该条评论的id
									"reuid" => $comments[$index]["repuid"],
									"msg" => $comments[$index]["msg"],
									"ctime" => api_dtktime($comments[$index]["time"]),
									//"img_url" => $img,
			);
		...
		$tmpResultBlogs = array(
			...
			"desc" => $tmpblog["desc"],
			"title" => $tmpblog["title"],
			"name" => $tmpblog["user"]["username"],
		    "domain" => $tmpblog["user"]["domain"],
			"ctime" => api_dtktime($tmpblog["time"]),
			"content" =>strip_tags(mb_substr($body["content"], 0, 700, "utf-8")), //140*5=700,大约显示140字的content预览
			//"blogs" => $blogs[$index],
			"hitcount" => $tmpblog["hitcount"],
			"commentcount" => $tmpblog["replaycount"],
			"likecount" => $tmpblog["feedcount"],
			"avatar" => get_avatar3($tmpblog["user"]["uid"]),
			"type" => $tmpblog["type"],
			"img_url" => getBigImg($imgso["url"]),
			"good" => $tmpblog["good_num"],
			"bad" => $tmpblog["bad_num"],
			"tag" => $tmpblog["tag"],
			"comments" =>$tmpResult
		);
			...
			if($this->spArgs('submit')) //预处理
			{
				...
				if(is_array($_FILES))
				{
					foreach($_FILES as $k=>$d)
					{
						...
						if($d['error'] != 4)
						{
							if($this->dtk['theme_upload'] != 1){exit('系统没启用上传功能');}
							if($d['size'] > $this->dtk['theme_uploadsize']){exit('上传大小不能超过：'.($this->dtk['theme_uploadsize']/1024/1024).'MB');}
							$ext = spExt('aUpload');
							$savedir = $ext['savepath'].'/'.$ext['savedir'].'/theme/';
							$fpath   =  date('Y').'/'.date('m').'/'.date('d').'/';
							$ext = pathinfo($d['name']);
							$exrarr = explode(',',$this->dtk['theme_uploadtype']);
							if(!in_array($ext['extension'],$exrarr)){exit('只能上传：'.$this->dtk['theme_uploadtype']);}
