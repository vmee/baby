...
function writeConfig()
{
	...
	if(!$sql = file_get_contents('tpl/ditieker.sql')) {exit('无法读取数据库模板文件tpl/ditieker.sql');}
	 $sql = str_replace('dtk_',$_POST['pre'],$sql);
...
function setUserInfo()
{
	...
	*/
	...
	$sql = "INSERT INTO `".$pre."setting` (`name`, `val`) VALUES
										...
										('site_desc', '地铁客轻博客开源上市'),
										('site_count', ''),
										('site_titlepre', '{$po['site_titlepre']}'),
										('site_keyword', '地铁客,云边,ditieker,轻博客,开源,轻博,ditieker,开源轻博,开源轻博客'),
										('site_icp', ''),
										('db_switch', '1'),
										('openlogin_qq_appid', ''),
										('openlogin_qq_appkey', ''),
										...
										('keep_email', 'admin,ditieker'),
										('keep_niname', 'ditieker,admin,administrator,master,webmaster,email,username,password'),
										('keep_domain', 'www,ditieker,bbs,music,map,index,register,login,tag,now,admin,recommend,discovery,myfollow,mypost,mylikes,myreplays,mynotices,edit,logout,home,gomember,location,showinfo,about,copyright,call,service,privacy,custom,read'),
										('keep_rep', '操你妈,艹你妈'),
										('show_page_mode', '1'),
										('show_ajax_to', '4'),
										('show_page_num', '10');
