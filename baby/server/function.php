...
//	if($d['repto'])
//	{
//		echo '<div class="repto"> 自<a href="'.goUserHome(array('domain'=>$d['repto']['domain'],'uid'=>$d['repto']['uid'])).'" target="_blank">'.$d['repto']['username'].'</a>收集到<a href="></a></div></div>';
//	}
	if($type == 6) //一句话笑话
	{
	//只有limit不显示全部的时候才显示
	...
	if($type == 3) //图片
	{
		if($limit == 'all')//如果显示全部
		{
			...
			{
				if(substr($d['url'],strlen($d['url'])-3,3)=="gif"){//如果是gif的就直接显示原图，否则显示t_的缩略图
					$img.= '<a class="highslide" href="'.$site_uri.'/'.getBigImg($d['url']).'" onclick="return hs.expand(this)"><img src="'.$site_uri.'/'.getBigImg($d['url']).'"  alt="'.$d['desc'].'" class="feedimg"/></a>';
				}else{
...
spAddViewFunction('feeds2','feeds2'); 
...
spAddViewFunction('getBigImg','getBigImg'); 
...
function feddshtml($blogs,$index=1,$type='index')
{
	...
	$msg = '';
		...
		foreach($feeds as $d)
		{
			...
			if($d['btype'] == 1)//文字
			{
				$msg .= '<div class="box"  style="background:url('.$d['img'].') no-repeat center top #425E6F;">
			...
			if($d['btype'] == 2) //音乐
			{
				if($d['metype'] != 'local')
				{
					if(strpos($rs['img'],'yinyuetai.com'))
					{
						$img = '<img src="'.array('src'=>base64_encode($rs['img'])).'"  alt=""/>';
					}else{
						$img = '<img src="'.$d['img'].'" width="180" alt=""/>';
					}
...
spAddViewFunction('dtktime','dtktime'); 
...
/*将正文和属性剥离出来分别存放*/
function split_attribute10($body)
{
	$body = preg_replace('/[\n\r\t]/', '', $body);
...
spAddViewFunction('splitattr','split_attribute'); 
...
function get_avatar($uid) {
	...
	$dir2 = substr($uid, 3, 2);
	$dir3 = substr($uid, 5, 2);
	$tmp_avatar = $dir1.'/'.$dir2.'/'.$dir3.'/'.'middle_'.substr($uid, -2).".jpg";
	$avatar_url = 'avatar/'.$tmp_avatar;
	
	if(file_exists(APP_PATH.'/'.$avatar_url)) {
		$avatar_url = $tmp_avatar;
	}else{
		$avatar_url = '/noavatar_middle.gif';
	}
	return $avatar_url;
}

function get_avatar2($uid) {
	$uid = sprintf("%09d", $uid);
	$dir1 = substr($uid, 0, 3);
	$dir2 = substr($uid, 3, 2);
	$dir3 = substr($uid, 5, 2);
	$tmp_avatar = $dir1.'/'.$dir2.'/'.$dir3.'/'.'small_'.substr($uid, -2).".jpg";
	$avatar_url = 'avatar/'.$tmp_avatar;
...
function get_avatar3($uid) {
	$uid = sprintf("%09d", $uid);
	$dir1 = substr($uid, 0, 3);
	$dir2 = substr($uid, 3, 2);
	$dir3 = substr($uid, 5, 2);
	$tmp_avatar = $dir1.'/'.$dir2.'/'.$dir3.'/'.'big_'.substr($uid, -2).".jpg";
	$avatar_url = 'avatar/'.$tmp_avatar;
...
function tagcloud(){   
    ...
    $maxFontSize=23; //最大字体大小,可根据需要自行更改
...
spAddViewFunction('tagcloud','tagcloud');
...
function collect($params)
{
	...
	if($type == 1 || $type == 6) //文字
	{
		$content = utf8_substr(strip_tags($content,'<p><ul><li><br><img>'),0,200);
		echo $content;
	}
	...
	if($type == 3) //图片
	{
		echo '<div class="preview"><a class="img" href="'.getBigImg($attr['img'][0]['url']).'" onclick="return hs.expand(this)"><img src="'.$site_uri.'/'.$attr['img'][0]['url'].'" class="feedimg" alt="'.$attr['img'][0]['desc'].'"/></a><p>'.$attr['img'][0]['desc'].'</p></div>';
	}
...
spAddViewFunction('collect','collect');


/**
 *
 */
function spOutput($content, $status=200, $mime_type="text/json"){
 ...
 * @return String
 */
function spEncode($string = '', $skey = 'babyphp') {
 ...
 * @return String
 */
function spDecode($string = '', $skey = 'babyphp') {
