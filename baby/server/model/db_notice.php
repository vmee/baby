...
{  
	...
	/*评论回复列表*/
	function apinoticeReplay($row,$title,$msg='')
	{
		$this->create(array('uid'=>$row['foruid'],'sys'=>'1','foruid'=>$row['uid'],'title'=>$title,'info'=>$msg,'time'=>time(),'location'=>'blog|'.$row['bid'],'time'=>time()));
		$this->sendReplay($row['uid'],$row['foruid'],$msg,$row['bid']);
	}
	/*发送私信
		进行严格私信限制
	*/
	...
	function sendReplay($imuid,$uid,$msga,$bid)
	{
		...
		$tpl = '<p>亲爱的'.$rs['username'].'：</p>
				...
				'.$for['username'].'回复了您的轻博客：</p>
				<p>"'.$msg.'"</p><a href="'.$uri['dirname'].goUserBlog(array('bid'=>$bid)).'" target="_blank">你可以立即查看您的轻博</a></p>';
