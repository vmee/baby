...
{  
	...
	function apicreateReplay($row)
	{
		...
		if( $GLOBALS['YB']['keep_rep'] != '')
		{
			...
			{ 
				$result['state'] = false;
				$result['msg'] = "含有敏感字符不允许发布";
				return $result;
			}
		}

		if($repuid != $row['uid'] && $repuid != '')
		{
			spClass('db_notice')->apinoticeReplay(array('foruid'=>$row['repuid'],'bid'=>$row['bid'],'uid'=>$row['uid']),'回复了您的博客',$msg);	  //给@发一个通知
		}else{
			spClass('db_notice')->apinoticeReplay(array('foruid'=>$rs['uid'],'bid'=>$row['bid'],'uid'=>$row['uid']),'回复了您的博客',$msg);     //给作者发
		}

		$this->create(array('bid'=>$row['bid'],'msg'=>$msg,'uid'=>$row['uid'],'repuid'=>$repuid,'time'=>time()));
		spClass('db_blog')->incrField(array('bid'=>$row['bid']),'replaycount'); //增加回复统计
